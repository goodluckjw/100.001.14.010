# 매칭된 내용이 있지만 chunk_map에 추가되지 않은 경우
        ...
        # 부칙에서만 검색어가 발견된 경우
        if found_in_부칙 and not chunk_map:
            print(f"참고: {law_name}에서는 부칙에서만 검색어가 발견되었습니다.")
            skipped_laws.append(f"{law_name}: 부칙에서만 검색어 발견됨")
            부칙_검색됨 = True
            continue
        ...
        for rule, locations in rule_map.items():
            ...
            if len(locations) > 1 and "각각" not in rule:
                # '한다.' 앞에 '각각'을 추가
                rule_parts = rule.split(" 한다.")
                rule_with_each = f"{rule_parts[0]} 각각 한다."
        ...
        if consolidated_rules:
            ...
            # 각 개정 규칙마다 줄바꿈 추가하여 가독성 향상
            amendment += "\n".join(consolidated_rules)
    ...
    return amendment_results if amendment_results else ["⚠️ 개정 대상 조문이 없습니다."]import requests
...
def group_locations(loc_list):
    ...
    for loc in formatted_locs:
        # 호, 목이 있는 경우와 없는 경우 구분
        if "호" in loc or "목" in loc:
            subitem_goal.append(loc)
        else:
            article_para.append(loc)
    
    result = []
    if article_para:
        result.append(", ".join(article_para))
    if subitem_goal:
        result.append("ㆍ".join(subitem_goal))
