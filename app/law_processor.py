...
def consolidate_locations_by_rule(locations, rule):
    """같은 개정 규칙을 가진 위치를 통합"""
    # 위치 정보 정렬
    sorted_locs = sorted(locations)
    
    # 위치 정보 그룹화
    loc_str = group_locations(sorted_locs)
    
    # 2개 이상의 위치를 묶는 경우 '각각'을 추가
    if len(locations) > 1:
        return f"{loc_str} 중 {rule.replace('한다.', '각각 한다.')}"
    else:
        return f"{loc_str} 중 {rule}"
